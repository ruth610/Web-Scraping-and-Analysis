...
for bank in df['bank'].unique():
    ...
    scores = list(zip(terms, bank_vec.A1))
    topn = sorted(scores, key=lambda x: -x[1])[:40]
    print(bank, topn[:20])
...
def assign_themes(text):
    ...
    if any(
        k in t
        for k in ['slow', 'loading', 'delay', 'transfer', 'timeout']
    ):
    ...
    if any(
        k in t
        for k in ['ui', 'interface', 'design', 'navigation', 'easy', 'confusing']
    ):
        themes.append('UI/UX')

    if any(
        k in t
        for k in ['support', 'help', 'customer service', 'agent', 'response']
    ):
